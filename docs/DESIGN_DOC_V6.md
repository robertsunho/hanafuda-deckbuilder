# Hanatu — Design Document V6

*Version 6 — 2026-06-15; supersedes V5*
...
This document supersedes V1 (Feb 2026), V2, V3, V4 (Mar 2026), and V5 (May 2026). It reflects the current state of the game as of June 2026, after a substantial systems-implementation phase, UI overhaul, codebase cleanup chapter, and the Phase-4 doc-reconciliation campaigns. Earlier versions are preserved as historical artifacts but should not be treated as authoritative.

V6 serves two audiences: (1) future Claude sessions picking up work after compaction, and (2) Robert's own thinking artifact. External-facing materials should be derived from this doc with appropriate filtering.
...
- 7.12 Tier 2 Fusion Spirits
- 7.13 Tier 3 Cross-Fusion Spirits
- 7.14 Tier 4 Capstone Spirits
- 7.15 Symbiont Spirits
...
- Earth (Clay/Pottery) — generates ki interest when held in hand at round end; held-in-hand mult channel is ×1.0 except under boost_earth (hex_15)
...
- Blue Stamp — discarded trigger: gain a free consumable when discarded (any discard pathway)
...
The Legendary spirit roster includes 4 spirits — the Tier 4 capstones (detailed in Section 7).
...
- Earth-enhanced cards in hand (Clay/Pottery) have a held-in-hand mult channel, but it is ×1.0 (no effect) except under the `boost_earth` hexagram (hex_15); Earth's main payoff is round-end ki interest (see Section 8.2.2).
...
- Irrigation (`util_irrigation`): each captured plain permanently gains +3 base points — a card mutation that benefits the plain's future captures (see Section 7.9)
...
|---|---|---|
| `volatile_flow` (hex_64, Wèi Jì) | Push-curve amplifier ×1.5 (success and failure), decay ×0.85 | Amplifies all push swings (see §9.3.8) |
| `stable_flow` (hex_63, Jì Jì) | Push-curve amplifier ×0.5 (success and failure), decay ×0.98 | Damps all push swings (see §9.3.8) |
...
**Stack counts (linear).** A spirit's `stackCount` (acquired via duplicate copies) multiplies its contribution *linearly*: a stack of N contributes N× the singleton contribution, NOT the singleton compounded N times. For multiplicative spirits the per-card scoring loop is the reference — `mult *= multiplyMult × count` — so Yang/Yin and the other cross-fusions, Wolf, the conditionals (Horizon/Dream/Hierarchy), and the multiplicative accumulators all scale as `base × stacks`. A 3-stack spirit applies its effect 3× per relevant trigger. (Known exception: the Mirror/Memory meta-spirits' `^n` copy-scaling is a separate deviation tracked under its own deferred recon — Mirror/Memory are not yet linear-consistent.)
...
Spirits are the primary build element of a Hanatu run. The player equips up to 6 spirits (default) plus 2 legendary slot spirits, drawn from a roster of **110 spirits** across multiple tiers and acquisition paths.
...
- **Wayside Shrine spirits (Tier 1, 78 spirits):** Available in shops throughout the run. Most spirits fall here — foundation, engines, utility, economy, gameplay, meta, decay, retrigger, conditional, and demoted rares. Cost ranges 3-8 ki. Rarity ranges common to rare.
...
The player's standard 6 spirit slots can hold spirits of any tier 0-3. The 2 legendary slots are exclusive to Tier 4 capstones (4 spirits total: the 4 capstones).
...
| Natural — 4th copy reaches stack (shop reroll, etc.) | 4 stacks | 4× base (lossless) |
| Sulfur duplication of a 3-stack (pushes to 4) | 4 stacks | 4× base (lossless) |

For natural transcendence (4-stack accumulation), `powerLevel = stackCount`: all 4 copies contribute and the Negative captures the full 4-stack state — transcendence is **lossless** (F4.26 Option B; it frees a slot while keeping full power). This unifies natural and Amber transcendence: both build the Negative through the same `_buildTranscendedNegative` path at the stack value present when transcendence fires (Amber at 1/2/3, natural/Sulfur at 4).
...
- **Data model.** A Negative is stored as `{ stackCount: 1, powerLevel: N }` — a singleton carrying baked power N (the stack value at transcendence). `effectivePower(spirit)` returns `powerLevel` for Negatives and `stackCount` for regulars; all engine/tooltip code reads `effectivePower`.
- **Tooltip header.** Negatives display `Negative (power ×N) — zero-slot`.
- **Sale refund.** A shop-bought Negative refunds `floor(cost × 0.5 × powerLevel)`.
...
- **Sulfur is RNG amplifier:** Duplicating an existing 3-stack with Sulfur pushes it to 4 and triggers natural transcendence at 4× base power (lossless); on lower stacks, it bumps the stack count toward the 4-stack threshold
...
| ID | Name | Effect | Cost | Rarity |
|---|---|---|---|---|
| `cond_horizon` | Horizon | ×2 mult per stack if capture contains both air and land cards (stacks add linearly: ×2 / ×4 / ×6) | 7 | uncommon |
| `cond_dream` | Dream | ×2 mult per stack if capture contains both day and night cards (stacks add linearly: ×2 / ×4 / ×6) | 7 | uncommon |
| `cond_hierarchy` | Hierarchy | ×1.5 mult per unique rank in capture (compounds within a capture; stacks add linearly) | 7 | uncommon |
...
**Hierarchy** rewards rank diversity within a capture. A 4-card capture with all 4 ranks (bright + animal + ribbon + plain) gets ×1.5⁴ = ×5.0625 mult. In the base deck, only **November** has all 4 ranks (Rainman bright + Swallow animal + Willow ribbon + Lightning plain), making it the natural target month for Hierarchy builds. Other months reach all-4-rank composition only with speculative cards integrated, or via card movement effects that bring foreign rank cards into a single month's stack. Stacking is linear: a 2-stack Hierarchy contributes 2× the single-copy value — the `1.5^ranks` rank-diversity term is computed within a single capture, and the stack count multiplies that result rather than raising the exponent.
...
| `game_echo` | Echo | First card scored in each capture retriggers 2× per stack (3 total fires at 1 stack) | 4 | common |

Stacking retriggers compounds linearly — 2 stacks of Rainbow gives 2 extra triggers per bright, scoring it 3 times total. Combined with rank engines (Radiance, Devotion), retriggers can produce massive per-bright burst. Because each retrigger re-runs the complete Phase 2 pipeline, an active hexagram's season/axis `onCardScored` multiplier is included on every retrigger of a qualifying card — not just its first scoring.
...
**Echo** retriggers the first card scored in **each capture** — 2 extra fires per stack (3 total fires at 1 stack, 5 at 2 stacks). Unlike rank-specific retriggers (Rainbow, Family, etc.), Echo is rank-agnostic — it re-scores whatever the player captures first in each capture event. Strategic implication: setting up a high-value first card (full month, multi-card with rich enhancements) maximizes Echo's value.

**Retrigger architecture (unified, additive).** Spirit and stamp retriggers are NOT separate systems — they compose through one accumulator, `_computeRetriggerCount(card, triggerType, …)`: White Stamp adds +1, Gray adds +3, then each spirit's `getRetriggerCount` is **added** on top. Composition is additive, not multiplicative — a Dew (+1) on a White-stamped (+1) plain scores **3** times total, not 4. Retriggers are scoped by **trigger type** — `'capture'` (rank retriggers Rainbow/Family/Wish/Dew, and Echo), `'held_in_hand'` (Applause), `'discard'`, `'yaku'` — and each spirit fires only for its own type. Mirror and Memory inherit their target's trigger scope by delegation (they pass the same `triggerType` through to the spirit they copy). All retriggers re-run the full Phase 2 per-card pipeline in the **same `points`/`mult` scope** (F2.10b) — including the active hexagram's `onCardScored` multiplier on every fire (F2.10c) — so multiplicative spirits **compound** across the fires: a Gray-stamped bright with Yang (×2) re-scored over 3 fires reaches ×8.
...
| Plain | Salt | Agriculture | Plenty | Irrigation |
...
| `util_symbiosis` | Symbiosis | Capturing an animal summons that animal's canonical symbiont, stacked at Symbiosis's stack count (stacks beyond 3 cascade-transcend into a Negative) | 7 | uncommon | Animal |
| `util_festival` | Festival | Capturing a colored ribbon generates a stamp of that color (per stack, slot-gated) | 5 | uncommon | Ribbon |
| `util_irrigation` | Irrigation | Each plain captured permanently gains +3 base points (mutates the card; stacks across captures) | 5 | uncommon | Plain |
...
- **Festival's slot-gating is its only generation cap.** Festival generates a stamp of the captured ribbon's color — one per stack, per ribbon captured — hard-gated by free consumable-inventory slots, stopping when the inventory is full. There is no separate per-round cap or cooldown; the slot gate is the cap. This makes it a strong spirit only when paired with consumable slot expansion.
- **Symbiosis's stack mechanic:** the stack count is the *stack* the summoned symbiont arrives at, not a variety multiplier. Capturing an animal summons that animal's single canonical symbiont (`ANIMAL_SYMBIONT_MAP`) at Symbiosis's stack count; stacks beyond 3 cascade-transcend the symbiont into a Negative copy. This is documented further in Section 7.15 (Symbionts).
- **Irrigation** (`util_irrigation`, `category: utility_card`) is the rank-utility spirit for plains. Its effect — a permanent per-card +3 base points each time the plain is captured — creates a long-tail accumulation on individual plain cards, transforming them into miniature engines themselves. A plain captured 5 times across a run has +15 permanent base points, making it more valuable than a fresh plain in subsequent captures. (Historical note: two Irrigation copies once coexisted — an `engine_irrigation` with the correct effect but wrong name, and a `util_irrigation` with a stale +10-ki effect; the code consolidated them into the single correct `util_irrigation` documented here.)
...
**Northern Lion** grants a free shop reroll on each successful push, converting push-your-luck aggression into shop-selection power. It rewards builds that push frequently and lean on shop access to assemble their loadout.

#### 7.10.3 Additional Economy Spirits

**Replica** and **Collector** are functional in the codebase, though their `description` fields still read "Coming soon" — a label cleanup item; the mechanics described here are the working design intent. **Print is non-functional** — it carries `rarity: null` and is removed from the shop pool pending its F5.9 implementation (a per-round consumable applier).
...
| ID | Name | Effect | Cost | Rarity |
|---|---|---|---|---|
| `game_catcher` | Catcher | Cards that would be discarded return to hand instead (up to N per round, N = stack count; gated on free hand space) | 4 | uncommon |
...
**Catcher** modifies card flow rather than scoring. In a build with frequent forced discards (Recycling, hexagram effects, full-field play), Catcher returns what would be lost cards to hand instead — up to N per round (N = stack count), gated on free hand space — net positive on most measures. (It uses an Osprey-style per-round budget: `catchesUsedThisRound` capped at the stack count.)

**Golden Toad** applies the Gold edition (+20 flat points) to scored cards during capture — up to N cards per capture at N stacks, skipping any card that already carries an edition. It pairs with Kintaro (Section 7.6.4), which consumes Gold for permanent mult-mult, forming a self-contained edition economy.
...
**Past Life maturity and Negatives (Category 5).** Past Life is a Category-5 maturity spirit (like Cuckoo Egg): copies are produced on sale only from elements that have *matured* — held at least 3 rounds (`CAT5_MATURITY_ROUNDS`), tracked per-element via `acquiredRound`. A regular Past Life fires one copy per matured element. A transcended (Negative) Past Life instead matures via an internal numerator/denominator counter whose **denominator = `powerLevel × CAT5_MATURITY_ROUNDS`**, so a higher-power Negative matures more slowly; once mature, selling it fires a copy at the Negative's own `powerLevel` (`_addPastLifeCopy`). Because Negatives never stack-merge, when the chosen target is itself a Negative the result is a fresh-zero parallel Negative copy at that power level; a regular target instead gains that many stacks (possibly cascade-transcending).
...
| `capstone_time` | Time | Push-curve amplifier ×1.5 success / ×0.5 failure (delta-from-neutral, scales with push depth); no inter-round flow decay |
...
Time modifies the Flow system itself rather than contributing to mult or points. It uses the push-curve **amplifier** architecture (shared with hexagrams hex_64/hex_63): rather than fixed multipliers, it scales each push outcome's delta-from-neutral, so the effect grows with push depth.

- Successful push: amplifier ×1.5 on the curve delta — `1 + (baseMult − 1) × 1.5` (depth 1 ≈ ×1.15, depth 2 ≈ ×1.375, depth 3 ≈ ×1.675)
- Failed push: amplifier ×0.5 on the curve delta (depth 1 ≈ ×0.95, depth 2 ≈ ×0.90)
- **No inter-round flow decay** while Time is equipped (`getEffectiveFlowDecay` → 1.0)
- Compounds multiplicatively with the hex_64/hex_63 push-curve amplifiers

Time is exceptional for push-heavy playstyles because every push is more valuable and less risky, and the no-decay clause lets conservative builds retain Flow between rounds.
...
- Capstones occupy legendary slots (the player has 2 legendary slots; can hold any combination of the 4 capstones)
...
| `sym_cuckoo_egg` | April Cuckoo | Matures over 3 rounds; the player sells the matured egg to hatch a random Tier 2 fusion (slot-gated) |
...
| `sym_ducks` | August Geese | Counter starts at 0: +1 per deck-flip match, −1 per strand (floored at 0). Contributes ×(1 + counter × 0.2 × stacks) mult. Permanent (Phase-5 redesign candidate) |
...
- **Symbiosis stacking:** Symbiosis's stack count is the *stack* the summoned symbiont arrives at — not a variety multiplier. Capturing an animal summons that animal's single canonical symbiont at Symbiosis's stack count; stacks beyond 3 cascade-transcend the symbiont into a Negative copy.
...
- **Cuckoo Egg's special behavior:** This symbiont matures over 3 rounds; once mature, the player **sells** it to hatch a random Tier 2 fusion (slot-gated). The hatch fires from the sale path (`fireCat5SaleEffects` → `_fireCuckooHatch`), not automatically at round end. During maturation the slot is occupied but provides no scoring contribution — a 3-round investment for a powerful payoff.
...
- **Permanent accumulators are per-element:** the run-permanent accumulator symbionts (Algae, Ants, Snails, Garden, Badger) track per-element state rather than a single flat counter — the tooltip shows the longest-held element's value and scoring aggregates across elements (see Section 5.7).
...
**Consume policy (used ⟺ spent ⟺ Badger).** A consumable is *used*, *spent*, and *counted by the Badger symbiont* as one indivisible event: this happens whenever it produces any real effect — including **partial** effects (a draw-2 that only finds 1 card, a multi-target where only some targets resolve) and **deliberate-cost** effects (Amber's −1 field slot, Tiger's forced push). It is **not** consumed only when it genuinely cannot act at all (no valid target, hand full, empty pile, pure no-op). "Used", "spent", and "Badger counts" are never decided separately — and this holds for alchemicals too (which previously consumed without counting for Badger; now unified). Implementation note: the decision lives in one shared predicate (`scenes/shared/consumePolicy.js` `consumableHadEffect`) consulted by every result-tail; the per-family result contracts remain as-is — element effects report via `action`, others via `success`.
...
| `chakra_crown` | Crown Chakra | Copy ALL attributes of one card onto another (rank, month, axes, enhancement, edition, stamp, and `baseImageId`) — the destination becomes an exact duplicate of the source. Targets 2 cards (source + destination) but converts only the destination; only the destination's deck-slot id is preserved | 2 |
...
- **Throat Chakra's duplication** copies all card properties exactly — including any existing enhancements, editions, or stamps. A duplicated card is a true clone, and (because the deck is mutated live) it is available the same round it is created.
...
- **Crown Chakra's copy** makes the destination card an **exact duplicate** of the source — ALL attributes copy (rank, month, axes, enhancement, edition, stamp, and the `baseImageId` so the sprite matches); the source card is unchanged. Only the destination is converted (1 card changes; 2 cards must be selected to define the operation), and only the destination's deck-slot id is preserved for tracking. The destination's prior enhancements, editions, and stamps are **overwritten**, not retained.
...
**Leaf / Silk (Wood):** Leaf creates a temporary field slot when all regular slots are full, allowing the card to play to the field rather than discarding. Silk additionally prevents stranding of any pending stack it is part of — per-card, not field-wide — in both the deck-flip case and the hand-play case (a 3-stack formed in hand that contains a Silk card resolves to a capture instead of stranding). The protected capture resolves in the deck-flip phase. A Silk card placed alone is not auto-captured; it only resists stranding of a real pending stack. Wood enhancements also contribute to mult during scoring.
...
**Clay / Pottery (Earth):** Earth-enhanced cards generate ki interest at round end if held in hand — Clay 10% / Pottery 20% of current ki per Earth card held at round end. This makes Earth a passive economy enhancement (its value scales with how much ki the player holds) and feeds the Fossil engine spirit. Earth enhancements also have a held-in-hand mult channel during scoring (Phase 1 of the scoring pipeline), but it is ×1.0 (no effect) in ordinary play — only the `boost_earth` hexagram (hex_15) makes it nonzero (×1.2 base / ×1.5 upgraded).
...
| `stamp_blue` | Blue Stamp | Discarded | Gain a free consumable when discarded | 4 |
...
| ID | Name | Trigger | Effect | Cost |
|---|---|---|---|---|
| `stamp_orange` | Orange Stamp | Captured / Yaku | Captured → draw +1 card; Yaku → +3 ki | 6 |
| `stamp_green` | Green Stamp | Captured / Discarded | Captured → free consumable; Discarded → +3 ki | 5 |
| `stamp_purple` | Purple Stamp | Discarded / Yaku | Discarded → draw +1 card; Yaku → free consumable | 6 |

Secondary stamps can also be **crafted** by combining two primary stamps (in addition to Sacred Grove purchase) — via the `STAMP_MIX` matrix in `consumables.js` (the pair is order-independent):
- Red + Yellow → Orange
- Blue + Yellow → Green
- Blue + Red → Purple
...
| ID | Name | Trigger | Effect | Cost |
|---|---|---|---|---|
| `stamp_black` | Black Stamp | Captured / Discarded / Yaku | Split compound — Captured → free consumable; Discarded → draw +1 card; Yaku → +3 ki (each effect fires on its own trigger, NOT all-on-any) | 9 |
...
Each recipe combines stamps with **different trigger types** — Black's three-way split (capture / discard / yaku) is assembled from a primary + secondary that together span all three trigger conditions.

#### 8.4.4 Quaternary Tier (Crafted by Combining White + Black)

| ID | Name | Trigger | Effect | Cost |
|---|---|---|---|---|
| `stamp_gray` | Gray Stamp | Captured / Discarded / Yaku | Black's split compound PLUS a ×3 retrigger on each trigger (any effect tied to the card fires 3 additional times, 4× total) | 12 |

Gray Stamp is crafted by combining a White Stamp with a Black Stamp:
- White on Black → Gray

Gray is the highest-tier stamp. It carries Black's full split compound (free consumable / draw +1 / +3 ki across capture / discard / yaku) **and** adds a ×3 generic retrigger on each trigger — amplifying whatever other interactions the card has.
...
**Tier-aware effect shifting (the full matrix).** Stamp effects are split by trigger, and ascending a tier *shifts* effects between triggers (within-pair swap at Secondary, three-way rotation at Tertiary). The complete table:
...
| White | retrigger ×1 | retrigger ×1 | retrigger ×1 |
| Orange | draw +1 | — | +3 ki |
...
Black fires its three effects **split across** the three triggers (not all-on-any). Gray carries Black's split plus a ×3 retrigger on each trigger.
...
- **Shop purchases** — Primary stamps from any shrine; secondary stamps from Sacred Grove (secondaries are also craftable — see below).
- **Crafting** — Secondary stamps crafted by combining two primaries (Red+Yellow→Orange, Blue+Yellow→Green, Blue+Red→Purple); Tertiary (Black) by combining a primary + a secondary; Quaternary (Gray) by combining White + Black.
...
| `zodiac_cat` | Cat | Summon a random Common (Tier 1) spirit — dynamic pool (currently 27, by rarity, symbionts excluded) — to an open slot | 3 | spirit |
...
**Monkey/Horse and round flow.** Monkey (capturing a field slot) and Horse (refreshing the hand) resolve through the same capture-completion path as normal play. A yaku completed by a Monkey capture offers the same Bank/Push decision a deck-flip capture would, and emptying the hand or deck via Monkey/Horse ends the round (full teardown). While a Push is pending, a Monkey-completed yaku succeeds the push and an emptied hand fails it — identically to deck play. Neither silently bypasses the yaku/round-end flow.
...
- **Cat** summons a random **Common** (Tier 1) spirit from a dynamic pool (currently 27 commons, filtered by rarity, symbionts excluded — it grows as more commons are added) to an open slot. Provides a budget alternative to shop browsing — but the random target may not align with the player's build.
...
| `alch_pearl` | Pearl | Fuse 2 Tier 3 cross-fusions into a Tier 4 Capstone (Legendary slot). Consumes both T3 inputs | 50 |
...
**Pearl** is the endgame fusion path — combines 2 Tier 3 cross-fusions into a Tier 4 Capstone. Pearl **consumes** both Tier 3 inputs (one stack of each). Highest ki cost (50) but highest payoff. To keep a component for reuse, pre-stack it first (Jade, or a Mirror/Memory copy) so a spare copy remains after the fusion.
...
**One operation per use.** Every fusion alchemical performs exactly one operation per activation, regardless of input stack depth. Cinnabar fuses one stack from each input — a 2-stack pair yields one fusion plus one leftover stack of each input; Mercury de-fuses one fusion stack; Pearl consumes one of each Tier 3 input to make one capstone. To preserve components, stack them before fusing.

**Negative spirits and fusion (current build).** Only regular spirits can be used as Cinnabar/Pearl fusion inputs — Negative (transcended) copies cannot fuse (the fusion paths read the regulars-only spirit set). The one negative-aware fusion path is the Cuckoo Egg symbiont's hatch: a transcended (Negative) Cuckoo hatches a Negative Tier-2 fusion, while a regular Cuckoo hatches a regular one.
...
**Sulfur** duplicates a random occupied slot, then clears another random occupied slot. Net effect: shifts the loadout's composition randomly while increasing one stack. Sulfur is double-edged — the duplicated slot might be a desired build piece (good) or a low-priority spirit (wasteful), and the cleared slot is similarly random. **Edge case:** When only one spirit is equipped, no clearing happens — Sulfur becomes a pure duplication. Sulfur can push a 3-stack to a 4-stack and trigger natural transcendence (4× power Negative, lossless).

**Amber** is the precision transcendence path. It transcends **any** spirit at **any** stack count, producing a Negative whose power equals the input stack count (1× / 2× / 3× for 1-stack / 2-stack / 3-stack inputs) — the weaker Negative is the intended tradeoff for forcing transcendence early. Costs **-1 permanent field slot** (floored at 1 slot) as a tradeoff — Amber-driven builds commit to reduced field capacity for the rest of the run.

#### 8.6.3 Run-Wide Acquisition Path

**Lead** summons a random Rare spirit at the cost of half the player's current ki. Provides access to the rare-rarity spirit pool without browsing shops. Lead does **not** exclude rares the player already holds — if a rolled Rare is already at 3 stacks, adding it cascade-transcends into a new Negative copy (the pool is simply every Rare in the catalog). Variance is high — the random rare may not fit the build, and the half-ki cost is significant late-run.
...
**Cycle note:** Wu Xing hexagrams operate on the **destructive** cycle — each boosts its own element and weakens that element's destructive predator (e.g. `boost_metal` strengthens Metal and weakens Fire, since Fire destroys Metal). The generative cycle is reserved for the consumable upgrade system (§8.2.1).

#### 9.3.8 Push / Flow (2)

| Hex | Number | Name | Effect | Modification |
|---|---|---|---|---|
| `hex_64` | 64 | Wèi Jì (Before Completion) | `volatile_flow` | Push-curve amplifiers ×1.5 on both success and failure (bigger wins, worse losses); flow decay ×0.85 (faster decay) |
| `hex_63` | 63 | Jì Jì (After Completion) | `stable_flow` | Push-curve amplifiers ×0.5 on both success and failure (gentler wins and losses); flow decay ×0.98 (slower decay) |

Both use the push-curve amplifier architecture (`pushCurveSuccessAmplifier` / `pushCurveFailureAmplifier`), which scales each push outcome's delta-from-neutral rather than setting a fixed flow multiplier. `volatile_flow` amplifies all push-curve swings by 1.5× — bigger gains, bigger losses — and decays flow faster (×0.85). `stable_flow` damps them to 0.5× — smaller gains and losses — and decays flow slower (×0.98). Both compound multiplicatively with the Capstone Time spirit, which uses the same architecture.
...
| `hex_21` | 21 | Shì Kè (Biting Through) | `eight_spirits_graduated_tax` | Spirit slots expand to 8, but each round-end taxes 3 ki per spirit beyond 4 (Negatives count toward the total): 5 spirits = 3 ki, 6 = 6 ki, 7 = 9 ki, 8 = 12 ki |
...
|---|---|---|---|---|
| `hex_38` | 38 | Kuí (Polarising) | `bright_and_plains` | Deck rebuilt: 1 bright + 3 plains per month (second plain duplicated); animals and ribbons removed; speculative bright where a month lacks one. 48 cards; no yaku-threshold modifier |
| `hex_39` | 39 | Jiǎn (Limping) | `deck_36` | Deck rebuilt to 36: 1 bright + 1 animal + 1 ribbon per month (no plains; speculatives where needed); field +1 |
| `hex_20` | 20 | Guān (Contemplation) | `deck_60` | Deck expanded to 60 (1 bright + 1 animal + 1 ribbon + 2 plains per month, using speculatives); hand +1 |
| `hex_23` | 23 | Bō (Stripping) | `all_plains_doubled` | All brights, animals, and ribbons removed; each month's distinct plains duplicated → 4 plains/month. 48 cards |
...
| `hex_24` | 24 | Fù (Returning) | `start_50_ki_no_income` | Run starts with 50 ki; round-end ki income zeroed (no hand-card ki, interest, or base reward) — spirit/stamp/consumable ki is still earned |
...
| `hex_16` | 16 | Yù (Enthusiasm) | `no_hand_ki_plus_capture` | No hand-card ki bonus at round end; +3 ki per capture (Push/Bank decision works normally) |
| `hex_06` | 6 | Sòng (Conflict) | `push_ki_swing` | Push success grants +10 ki; push failure costs 10 ki |
...
Blessings are acquired exclusively at the Sacred Grove (mechanic specifics: not yet implemented; see Section 16 Implementation Status).
...
```
Ki Reward = 5 (base)                                       [round_end_base]
          + cards_in_hand × Piggybank multiplier           [round_end_hand]
          + earth_ki_bonus                                 [round_end_earth]
          + interest (pre-credit balance, push-scaled)     [round_end_interest]
          + hexagram modifyKiReward delta (hookDelta)      [round_end_hook_bonus / _penalty]
```

Style-combo ki is **not** part of the round-end reward — it is awarded mid-round, per combo (see §11.3). Round-end ki is credited as **five separately-logged components** (per-component logging, F3.9), each with its own reason string (shown in brackets above):

| Component | Reason | Description |
|---|---|---|
| **Base** | `round_end_base` | Flat 5 ki per round, regardless of performance |
| **Cards-in-hand** | `round_end_hand` | +1 ki per card remaining in hand × `(1 + piggybank_stacks)`. Forfeited entirely on push failure. No hard cap; soft-capped by build economy |
| **Earth ki** | `round_end_earth` | Interest from Clay/Pottery-enhanced cards held in hand at round end (10%/20% of current ki) |
| **Interest** | `round_end_interest` | `floor(ki × interest_rate × pushMult)` — computed on the **pre-credit** balance and scaled by the push-resolution multiplier (see §11.2) |
| **Hex hookDelta** | `round_end_hook_bonus` / `_penalty` | The `modifyKiReward` hexagram delta (`total − subtotal`); positive credited, negative spent |

Flow and interest are **push-driven**: they resolve on push outcomes (bank / fail), not passively — see §5.4 and §11.2.

**Push failure penalty:**

If the round ends due to push failure, the player forfeits the hand-derived ki (the `cards_in_hand × Piggybank` component). Base 5 ki still applies, and interest still resolves (push-scaled — a failure shrinks it; see §11.2). Style-combo ki was already credited mid-round (§11.3) and is unaffected.
...
- `start_50_ki_no_income` (Fù / Returning) — Run starts with 50 ki; round-end ki income zeroed (no hand-card ki, interest, or base reward) — spirit/stamp/consumable ki still earned
- `no_hand_ki_plus_capture` (Yù / Enthusiasm) — No hand-card ki bonus at round end; +3 ki per capture (Push/Bank works normally)
- `push_ki_swing` (Sòng / Conflict) — Push success grants +10 ki; push failure costs 10 ki

### 11.2 Interest (Round-End, Push-Scaled)

Interest is **not** a passive round-start bonus — it is computed at **round end**, on the **pre-credit** ki balance, and scaled by the push-resolution multiplier:

```
Interest = floor(ki × interest_rate × pushMult)
```

where `pushMult = getPushMultiplier(pushDepth, outcome)`. A deeper *successful* push amplifies interest; a push *failure* shrinks it. Interest is one of the five round-end ki components (§11.1), credited with reason `round_end_interest`. (The legacy round-start `applyInterest()` method is now dead code — see `CODEBASE_CLEANUP.md`.)

**Push-curve amplifiers also scale interest:** the same `pushCurveSuccessAmplifier` / `pushCurveFailureAmplifier` architecture used by hexagrams hex_64/hex_63 and the Capstone Time spirit (see §9.3.8) feeds `getPushMultiplier`, so those modifiers amplify round-end interest ki as well as Flow.
...
- Interest compounds across the run: each round's pre-credit balance is larger, so even the unchanged base rate produces exponential late-game growth — now modulated by push aggression (deep successful pushes amplify it, failures shrink it)
...
| Orange Stamp | +3 ki | On yaku contribution (its capture trigger gives a card draw, not ki) |
| Green Stamp | +3 ki | On discard (its capture trigger gives a free consumable, not ki) |
| Black Stamp | +3 ki | On yaku contribution (capture → consumable, discard → draw) |
| Iron / Meteorite jackpot | +30 ki | 5% chance per Metal-enhanced card capture |
| Recycling spirit | +5 ki per stack | Per discard event |
| Goat Zodiac (active) | +1 ki per capture | Round-long buff once activated |
| Style combo ki | +1 ki per combo (base), then chained through spirits | Mid-round, when a style combo first triggers (once-per-run) |
...
**Style-combo ki is mid-round and spirit-chained.** When a style combo first triggers, its base **+1 ki** is awarded immediately (mid-round, before any push attempt) and passes through the spirit chain in slot order — each spirit's `applyKiBonus` hook can modify it (e.g. Grace multiplies it by `1 + grace_stacks`). It is NOT part of the round-end reward formula (§11.1).
...
| Spirit | Effect |
|---|---|
| `econ_recycling` | +5 ki per discard event, per stack |
...
| `econ_replica` | Duplicates a consumable you own at the start of each round |
...
|---|---|---|
| Spirits (Q_TL) | Random Tier 1 | Random Tier 1 (same pool as Wayside; more slots, no legendary roll) |
...
Slot counts (field, hand, spirits, consumables, cards-dealt) are dynamic and **recomputed on demand** from the current `RunManager` state — mid-round mutations (hexagram effects, alchemicals such as Amber, the Rooster zodiac) propagate immediately to the scoring engine and the next render, not only between rounds.
...
- **Q_TL Spirits** — Tier 1 spirits (random from pool); Sacred Grove offers more slots from the same pool
...
════════════════════════════════════════════════════════════
Ki: 0 | Deck size: 48
Hexagram: Receptive Earth (坤 Kūn) [hex_02]
  Default hexagram for first runs; gameplay proceeds normally
...
The full 110 spirits across 12 categories. For full descriptions, see Section 7.
...
| Tier 1 (Common/Uncommon/Rare) | 78 | Wayside Shrine, Sacred Grove |
...
| **TOTAL** | **110** | (excluding Negatives) |

#### 15.2.2 Tier 1 Spirits by Category

For the full Tier 1 catalog organized by role, see Section 7.5–7.11.
...
| Rank Utility Spirits | 4 | Glory, Symbiosis, Festival, Irrigation (Section 7.9) |
| Economy Spirits | 12 | Recycling, Piggybank, Grace, Bonds, Ingot, Replica, Northern Lion (Section 7.10) |
| Gameplay & Meta Spirits | 5 | Catcher, Golden Toad, Mirror, Memory, Past Life (Section 7.11) |
| Decay Spirits | 2 | Persimmon, Pear (Section 7.6.5) |
| **Sum (categorized)** | **78** | |

The Section 7 categorization sums to 78, matching the 78 Tier 1 entries in `spirits.js`. (Engine includes the 5 former demoted rares — Wuji, Dao, Chi, Tengu, Feng Shui — now grouped under Miscellaneous Engines; Decay is listed as its own row; Northern Lion sits under Economy and Golden Toad under Gameplay & Meta, per their mechanical roles. See Section 7.)
...
| `chakra_crown` | Crown | Copy ALL attributes (exact duplicate) from source onto destination | 2 (1 converted) |
...
| `stamp_blue` | Blue | Primary | Discarded | Free consumable on discard | 4 |
...
| `zodiac_cat` | Cat | spirit | Summon random Common (T1) spirit | 3 |
...
| System | Status | Notes |
|---|---|---|
| Tier 1 spirits (78 total) | ✅ | All Tier 1 spirits functional |
...
| Irrigation consolidation | ✅ | Consolidated to a single `util_irrigation` (correct +3-points effect, correct name); the old `engine_irrigation` duplicate is removed |
...
| System | Status | Notes |
|---|---|---|
| Round-end ki reward formula | ✅ | Base 5 + hand × Piggybank + Earth + interest + hex hookDelta (combo ki is mid-round, not round-end) |
| Push failure penalty | ✅ | Forfeits hand-derived ki |
| Interest (round-end, push-scaled) | ✅ | `floor(ki × rate × pushMult)`; base 10% + Bonds + Ingot + hex |
