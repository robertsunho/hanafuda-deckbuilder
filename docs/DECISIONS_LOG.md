...
## D-ORIENT — Orientation consolidated into CLAUDE.md; local memory reduced to a derivative pointer (2026-06-21)

**Status:** DONE — orientation/docs only (no code, V6, or ARCHITECTURE *content* touched). The last
pre-Phase-5 foundation piece.

**Problem (root cause, not symptom):** Claude Code's local per-project memory file
(`~/.claude/projects/…/memory/MEMORY.md` — outside the repo, unversioned, loaded into context each
session) had become an **invisible second source of truth**: a fat standalone copy of architectural facts
that no one audited and that drifted badly. It asserted removed/renamed things — `calculateFinalScore`,
`SNOW_MULT`/`ICE_MULT` exports, Three Marks, Paramita shop layout, `_totalScore`/`_styleBase` fields,
`run.applyElement`, `_markMode`, the singular `playHandCard`, and the old "after rounds 3,6,9,12,15" Grove
cadence — and fed those stale facts into every session.

**Fix (structural, not cosmetic):** collapse to ONE visible, version-controlled orientation source.
- **`CLAUDE.md`** (versioned) gained a lean **"Architecture orientation"** section: the load-bearing facts
  a fresh session needs, as orientation **plus pointers** into the canonical docs (NOT a fat cheat-sheet —
  duplication is what rots). Every fact verified against the post-Gate-0 baseline (V6 + ARCHITECTURE +
  code).
- **Local `MEMORY.md`** reduced to a thin derivative pointer at the repo (`CLAUDE.md` + `ARCHITECTURE.md`
  + `DESIGN_DOC_V6.md` + `DOC_MAP.md`) with the rule: no architectural/code facts here (the repo records
  them); only durable non-repo facts (user prefs / feedback / goals) as lean entries. Nothing substantive
  left to drift.

**Step-0 mechanism finding:** the memory store was a single 8.4 KB `MEMORY.md` (no individual memory
files), maintained by the assistant via the Write tool — **not** auto-regenerated by a background process,
so it is stable once written and CAN be held as a thin pointer (Option 1, preferred). It is read into
context at session start, which is how the drift propagated unseen.

**No CHANGELOG** — `CLAUDE.md` is operational onboarding, not a canonical reference doc the CHANGELOG
