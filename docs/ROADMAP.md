...
| — | **id↔name divergence sweep** — reconcile display names drifted from canonical ids | G0 Humidity→Wet finding; D-GATE0-DOCFIX; Robert ruling | Systematic pass: per case, decide whether the **id** or the **display name** is canonical, and align. **Known seed:** the spirit id `summer_humidity` has display name **"Wet"** — "Humidity" (the id stem) is the deprecated term; the doc-campaign used the live "Wet" and flagged the divergence. **Sequencing:** same family as **D1 / Cand. C** (code-vocabulary rename, above) — names drifted from canonical identifiers; do them **together**, after the structure stops moving (per the §5 Candidate-C/I note). |
...
| **G0-006** | **Glory** redesign: stack-responsive draw (one use per copy, up-to-2 per use) | G0-006 / GATE_0_FINDINGS; Robert ruling | **Current (V6 §7.9):** flat draw **2** on each bright capture (deck-capped → 0/1 if the deck is short); the effect + V6 are flat-2 — the *tooltip* wrongly showed 2×stacks (a code bug). **Ruling:** all spirits should be stack-responsive, so flat-2 is the anomaly — Glory should scale with stacking, **but NOT a naive draws×stacks bulk draw**. **Mechanic:** each Glory copy grants **one use per trigger** (bright capture); each use draws **up to 2** — singleton = up-to-2 (the +2 is retained); 3-stack = 3 uses × up-to-2 = up to 6, **as discrete capped pulls, not one bulk draw of 6** ("if you only have 2 cards in hand you won't draw 6 at once"). **⚠ OPEN (resolve with Robert before BUILD):** what does "up to 2 per use" cap against? **(a)** a per-use flat cap limited only by deck availability — "uses" are an accounting structure (3 stacks reliably draws 6 if the deck has the cards; the only-2-in-hand case is really deck exhaustion); **(b)** a per-use cap that also respects a hand/draw constraint (hand capacity / available-to-draw) so it can't balloon the hand. **Tuning (→ Phase 6):** the magnitudes (the "2", the per-use count, whether 3-stack→6 is too strong) are balance — soft 5A-shape / 6-tune split. **Post-baseline:** V6 currently says flat-2 (the accurate current baseline); building this CHANGES behavior → a real CHANGELOG `[design]` entry + V6 §7.9 rewrite when it ships. |
| **G0-042** | **Card tag system** — use or remove | G0-042 / GATE_0_FINDINGS; Robert ruling | The card `tags` field was designed early and never went anywhere — the **only** card with non-empty tags is `december_plain_3` (the deprecated duplicate slated for removal, G0-041 / CODEBASE_CLEANUP), so the system has essentially no live data; V6 §2.8's claimed `cardsByTag` helper never existed (doc-corrected). **Decide:** give tags a real purpose + populate them, OR remove the system entirely (drop the `tags` field + the `cardsByTag` aspiration). **Related:** removing `december_plain_3` (G0-041) deletes the tag system's only live data point — "remove tags" and "delete `december_plain_3`" move together; also touches the id↔name / vocabulary cleanup (the sweep above + D1/Cand. C). |
